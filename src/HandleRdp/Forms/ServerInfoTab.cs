...
{
    private readonly RdpServerInfoRepository _repo = new();
    ...
    public ServerInfoTab()
    {
        ...
        _grid.AddToolbarButton("修改選取", (_, _) => EditSelected());
        _grid.AddToolbarButton("批次匯入(CSV)", (_, _) => ImportCsv());
        _grid.AddToolbarButton("刪除選取", (_, _) => DeleteSelected());
    ...
    private static List<Field> BuildFields(RdpServerInfo? src, bool hostnameReadOnly)
    {
        ...
        {
            new("Hostname", "Hostname", src?.Hostname ?? "", readOnly: hostnameReadOnly, required: true),
            new("Department", "Department", src?.Department ?? "", required: true),
            new("Category", "Category", src?.Category ?? "", required: true),
            new("Connectstring", "Connectstring", src?.Connectstring ?? ""),
            new("Sponsor", "Sponsor", src?.Sponsor ?? ""),
            new("Claim_Time", "Claim_Time", src?.Claim_Time?.ToString() ?? ""),
        };
    }

    private static RdpServerInfo ReadFields(FieldDialog dlg) => new()
    {
    ...
    private void EditSelected() => Ui.Guard(() =>
    {
